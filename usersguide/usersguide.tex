...
\[
\min_{\v{u}(t), \v{p}} \quad M(\v{x}(t_f), \v{u}(t_f), \v{p}, t_f) + \int_{t_0}^{t_f} L(\v{x}(t), \v{u}(t), \v{p}, t)\, \mathrm{d}t
\]
...
\ref{addMayer} and \texttt{model} \, \texttt{.addLagrange} \ref{addLagrange} or
with the method
...
analyzed with the presented framework \ref{c:Analysis}.
...
model.optimize(tf=1, steps=1, rksteps=1, flags={}, meshFlags={}, resimulate=False)
	...
	\begin{itemize}
		...
		\item \texttt{dict<str, arg> flags} \emph{(optional)}: A
		      dictionary of flags. All possible flags are presented in \ref{c:flags}.
		\item \texttt{dict<str, arg> mesh flags} \emph{(optional)}: A
...
\begin{mdframed}[backgroundcolor=gray!10, roundcorner=10pt,
	...
	\begin{itemize}
		...
		\item \texttt{"linearSolver" $\rightarrow$ LinearSolver}
		      ...
		      enumeration \ref{enum:LinearSolver}.
		...
		\item \texttt{"ivpSolver" $\rightarrow$ IVPSolver}
		      ...
		      \texttt{IVPSolver} enumeration \ref{enum:IVPSolver}.
...
\begin{mdframed}[backgroundcolor=gray!10, roundcorner=10pt,
	...
	\begin{itemize}
		...
		\item \texttt{"meshAlgorithm" $\rightarrow$ MeshAlgorithm}
		      ...
		      element of the \texttt{MeshAlgorithm} enumeration \ref{enum:MeshAlgorithm}.
...
\section{Analysis}
\label{c:Analysis}
The \texttt{optimization} package offers the possibility to directly analyze the optimal solution with  \texttt{pandas} and \texttt{Matplotlib} functions. To access the features described in this section, it is mandatory to set the \texttt{"outputPath"} flag \ref{flag:outputPath} in \texttt{model.optimize} \ref{optimize}. Otherwise, the solution will not be written to a file.

\subsection{Results}
\label{c:Results}
In any case the framework generates a metadata file "\textbackslash tmp\textbackslash modelinfo.txt", which includes basic information about the solving process. This file is automatically read in as a \texttt{dict} after a successful optimization and includes the time taken by the solver as well as the number of mesh iterations. The modelinfo can be accessed via \texttt{model.modelInfo}, e.g.
\begin{lstlisting}
...
If the \texttt{"outputPath"} flag \ref{flag:outputPath} is set, the framework can directly import the optimal solution from the back-end as a \texttt{pandas.DataFrame}. Note that the names of the variables are taken from the \texttt{symbol} argument in \texttt{addState} \ref{addState}, \texttt{addInput} \ref{addInput} and  \texttt{addParameter} \ref{addParameter}, if no \texttt{symbol} is provided a dummy name will be used.
...
\subsection{Plotting}
\label{c:Plotting}
The framework includes several functions to draw standard plots of states and inputs, sparsity patterns, parametric plots and of the mesh refinement process as well. 
...
model.plot(specifCols=None, meshIteration=None, interval=None, dots=Dots.OFF) 
	...
	\begin{itemize}
		...
		\item \texttt{Dots dots} \emph{(optional)}: Adds dots of the discrete solution to the plot. This has to be an element of the enumeration \ref{enum:Dots} and in the default case no dots are added.
	\end{itemize}
...
model.plotInputs(meshIteration=None, interval=None, dots=Dots.OFF)     
		\end{lstlisting}
			...
			\textbf{Example:} 	\begin{lstlisting}
model.plot(specifCols=["x", "v"], meshIteration=5, interval=[0.2, 0.3], dots=Dots.ALL)   
					\end{lstlisting}
\end{mdframed}
...
model.plotMeshRefinement(interval=None, markerSize=30, dots=Dots.BASE)
	...
	\begin{itemize}
		...
		\item \texttt{Dots dots} \emph{(optional)}: Sets which points are displayed in the plot. This has to be an element of the enumeration \ref{enum:Dots} (excluding \texttt{Dots.NONE}) and in the default case base points are added as a dot.
	\end{itemize}
...
The plot of the mesh refinement is not very useful on its own. It is therefore possible to combine it with standard plots. In this way, the sections that have been recognized as important or conspicuous by the  algorithm can be observed.
...
model.plotVarsAndRefinement(meshIteration=None, interval=None, specifCols=None, markerSize=30, dotsMesh=Dots.BASE, dotsGraph=Dots.OFF)
	...
	\begin{itemize}
		...
		\item \texttt{Dots dotsMesh} \emph{(optional)}: Sets which points are displayed in the refinement plot. This has to be an element of the enumeration \ref{enum:Dots} (excluding \texttt{Dots.NONE}) and in the default case base points are added as a dot.
		
		\item \texttt{Dots dotsGraph} \emph{(optional)}: Adds dots of the discrete solution to the standard plot. This has to be an element of the enumeration \ref{enum:Dots} and in the default case no dots are added.
	\end{itemize}
...
Additionally, the framework is capable of drawing sparsity patterns. To be able to draw a given sparsity pattern the flag \texttt{"exportjacobianPath"} \ref{flag:exportJacobianPath} or \texttt{"exportHessianPath"} \ref{flag:exportHessianPath} has to be set.
...
model.plotSparseMatrix(matrixType)
	...
	\textbf{Parameters:}
	\begin{itemize}
		\item \texttt{MatrixType matrixType}: Sets the matrix. This has to be an element of the enumeration \ref{enum:MatrixType}.
	\end{itemize}
...
piecewise functions. For code generation to work, this has to have the fallback
...
\begin{mdframed}[backgroundcolor=gray!10, roundcorner=10pt,
	...
	\ref{c:Plotting} Determines if and which values are added to the plot as a dot.
