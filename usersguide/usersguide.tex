...
This repository contains the entire framework, i.e. the front-end (Python modeling environment) and the back-end (C++ implementation of local collocation dynamic optimization with mesh refinements).

\subsection{Requirements}
Ensure that the following dependencies are installed:

	
TODO: make this better -> what is built automatically?! what is needed ...? 
install python package
HSL LIB\_HSL path ... env |grep LIB\_HSL
make the path of running models generic in cmake!
install:
sudo cp libgdopt.so /usr/local/lib/ 
sudo ldconfig
set GDOPT\_SOURCES
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item[$\rightarrow$] set the environment variable "LIB\_HSL"
	\end{itemize}
	...
	\begin{itemize}
		...
		\item[$\bullet$] pandas\cite{mckinney2010data} (\emph{optional}: only used for importing solutions)
		\item[$\bullet$] Matplotlib\cite{hunter2007matplotlib} (\emph{optional}: only used for plotting solutions)
	\end{itemize}
