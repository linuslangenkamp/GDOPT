...
Combined objectives can be added individually with \texttt{addMayer}
\ref{addMayer} and \texttt{addLagrange} \ref{addLagrange} or with the method
\texttt{addObjective}.
...
These helper expressions can also have the Python standard types
...
If the expressions provided by the user are not simplified already, it
...
continuous formulation of the GDOP \ref{def:GDOP} to a large scale nonlinear
...
InitVars(Enum)
	...
	\begin{itemize}
		...
		\item \texttt{SOLVE\_EXPLICIT\_EULER}: The initial
		      ...
		      guesses for $\v{u}(t)$
		      and $\v{p}$. The system is solved by an explicit /
...
MeshAlgorithm(Enum)
	...
	\begin{itemize}
		...
		\item \texttt{L2\_BOUNDARY\_NORM}: An advanced mesh algorithm,
		      ...
		      interval boundaries. Additionally this algorithm is
