...
Problem::Problem(int sizeX, int sizeU, int sizeP, std::vector<double> x0, std::vector<double> lbX, std::vector<double> ubX,
    ...
    : sizeX(sizeX), sizeU(sizeU), sizeP(sizeP), x0(std::move(x0)), lbX(std::move(lbX)), ubX(std::move(ubX)), uInitialGuess(std::move(uInitialGuess)),
      lbU(std::move(lbU)), ubU(std::move(ubU)), pInitialGuess(std::move(pInitialGuess)), lbP(std::move(lbP)), ubP(std::move(ubP)), M(std::move(M)),
      L(std::move(L)), F(std::move(F)), G(std::move(G)), R(std::move(R)), A(std::move(A)), name(std::move(name)) {
}
